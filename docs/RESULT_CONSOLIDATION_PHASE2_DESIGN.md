...
| D4 | **TC ID 프리픽스 = 플랫폼** | `SC-` = PC Web, `SCM-` = 모바일 웹 (조직 규칙, 추후 추가 가능). 매핑은 서버 설정 테이블 — 미등록 프리픽스는 프리픽스 문자열 그대로 라벨. **운영 방식: 플랫폼별로 프로젝트를 분리**(혼합 안 함, 사용자 확정) → 플랫폼 축은 Phase 2 범위 제외. 대신 **미리보기에서 프리픽스 불일치 경고**(기존 TC 와 다른 플랫폼 파일 업로드 시 "매칭 0건 예상" 안내)로 활용. `record.platform` 은 저장(향후 대비). |
...
{ id, sourceId, projectId,
  ...
  platform: 'pc-web|mobile-web',          // TC ID 프리픽스에서 자동 유도 (D4) — Playwright 레코드에도 소급 적용
...
4. 축 집계(`computeAxes`)에 매뉴얼 `envResults` 기반 **환경축** 추가(소스 2종 이상일 때 노출 — 기존 규칙 승계).
   플랫폼축은 범위 제외(D4 — 플랫폼별 프로젝트 분리 운영).
...
| ③ 미리보기 | preview API 결과: 시트별 인식 행 수, 기존 TC 매칭 N / 신규 X / 예상 불일치 통계, 샘플 10행. **프리픽스 불일치 경고**(D4 — 기존 TC 와 플랫폼이 다르면 "매칭 0건 예상" 안내) | |
