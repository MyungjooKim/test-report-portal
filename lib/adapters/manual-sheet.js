...
function analyzeSheet(sheet) {
  ...
  return { adopted: true, tcIdCol, resultCols, naReasonCol, titleCol, rowCount: validRows.length };
}
...
function parse(input, opts = {}) {
  ...
  for (const sheet of sheets) {
    ...
    for (const row of sheet.rows) {
      ...
      if (!byTc.has(tcId)) byTc.set(tcId, { envResults: [], naReasons: [], titles: [], sheets: new Set() });
  ...
  for (const [tcId, acc] of byTc) {
    ...
    records.push({
      ...
      reasonNote: result === 'N/A' ? (acc.naReasons[0] || null) : null,
...
module.exports = { supports, parse, analyzeSheet, findTcIdCol, classify, representative, platformOf, TC_ID_RE, PLATFORM_BY_PREFIX };
