...
function analyzeSheet(sheet) {
  ...
  return { adopted: true, tcIdCol, resultCols, naReasonCol, titleCol, priorityCol, rowCount: validRows.length };
}
...
function parse(input, opts = {}) {
  ...
  for (const sheet of sheets) {
    ...
    for (const row of sheet.rows) {
      ...
      if (!byTc.has(tcId)) byTc.set(tcId, { envResults: [], naReasons: [], titles: [], priorities: [], sheets: new Set() });
