...
//   하나만 있으면 → 그 값
//   예외: 'test-run' 소스(수행 보드 발행 최종)가 있으면 그 값이 권위 — 보드의 커버리지 인지
//   파생(부분 커버리지 자동 Pass 단독 = N/T 등)을 D 규칙이 덮어쓰지 않도록 한다.
//   다른 결정 소스가 권위값과 다르면 mismatch 표시는 유지(최종은 권위값).

// 심각도(높을수록 우선) — 한 소스 안에서 같은 TC가 여러 결과일 때 worst-wins.
// N/A(테스트 범위 밖, D1)는 최약 — 다른 값이 하나라도 있으면 그 값이 이긴다.
// Inconclusive: 멀티태그로만 커버돼 이 소스 결과로는 판정 불가 (N/T 계열, 결정값 아님).
const SEVERITY = { 'Fail': 4, 'Blocked': 3, 'Pass': 2, 'N/T': 1, 'Inconclusive': 0.5, 'N/A': 0 };
const DECISIVE = new Set(['Pass', 'Fail', 'Blocked']);

// 한 소스 셀의 자동화 결과 판정:
//  - 이 TC 를 단독 검증한 결과(soloResults)가 있으면 그것만으로 worst-wins (멀티커버는 무시)
//  - 단독 검증이 없고 멀티커버만 있으면 Inconclusive (그 테스트 fail 이 이 TC 탓인지 알 수 없음)
//  - 멀티 정보가 아예 없으면(매뉴얼 등) 기존대로 전체 worst-wins
function cellVerdict(cell) {
  if (cell.soloResults && cell.soloResults.length) return worst(cell.soloResults);
  if (cell.hasMulti) return 'Inconclusive';
  return worst(cell.results) || 'N/T';
}

// 사유 자연어 번역 — 표의 사유 컬럼용. 전체 에러 텍스트(errorDetail)로 번역해야
// 요소명·기대/실제 값까지 살아난다(첫 줄 reasonNote 만으론 문장이 밋밋해짐).
const { humanizeError } = require('./error-humanize');
...
function consolidate(records, opts = {}) {
  ...
  const addToGroup = (key, rec, exchange) => {
    ...
    if (!g.sources.has(source)) {
      g.sources.set(source, { results: [], soloResults: [], hasMulti: false, reasons: [], errorTexts: [], flaky: false, attachments: 0, titles: new Set(), envResults: [] });
    }
    const cell = g.sources.get(source);
    if (rec.result) {
      cell.results.push(rec.result);
      // 멀티태그(한 테스트가 여러 TC 커버) 구분 — 이 TC 를 단독 검증한 결과만 soloResults 에 모은다.
      // 단독 검증이 하나도 없으면(멀티커버뿐) 이 결과들로는 TC 판정 불가 → Inconclusive.
      if (rec.coveredByMulti) cell.hasMulti = true;
      else cell.soloResults.push(rec.result);
    }
    if (rec.reasonNote) {
      cell.reasons.push(rec.reasonNote);
      cell.errorTexts.push(rec.errorDetail || rec.reasonNote); // reasons[i] 와 정렬 유지
    }
  ...
  for (const g of groups.values()) {
    ...
    for (const [src, cell] of g.sources) {
      const agg = cellVerdict(cell);
      sources[src] = {
        result: agg,
        flaky: cell.flaky,
        reason: cell.reasons[0] || null,
        humanReason: cell.errorTexts.length ? humanizeError(cell.errorTexts[0]) : null, // 미지 패턴은 null(원문만)
    ...
    const auth = sources['test-run']; // 수행 보드 발행 최종 — 권위값
    if (auth) {
      final = auth.result || 'N/T';
      // 권위값이 결정값일 때만, 다른 소스의 결정값이 어긋나면 불일치 표시 (최종은 권위 유지)
      const others = Object.entries(sources)
        .filter(([src]) => src !== 'test-run')
        .map(([, cell]) => cell.result)
        .filter(v => DECISIVE.has(v));
      mismatch = DECISIVE.has(final) && others.some(v => v !== final);
    }
    else if (uniqDecisive.length === 0) {
      // 결정값(Pass/Fail/Blocked) 없음 — 우선순위: N/A(전부 범위 밖) > Inconclusive(판정 불가) > N/T
      if (aggVals.length && aggVals.every(v => v === 'N/A')) final = 'N/A';
      else if (aggVals.includes('Inconclusive')) final = 'Inconclusive';
      else final = 'N/T';
    }
...
function addFinal(c, final) {
  // 범위 밖(N/A)·판정 불가(Inconclusive)는 축 분포·모수에서 제외 (모수 제외 원칙과 일관)
  if (final === 'N/A' || final === 'Inconclusive') return;
...
function computeStats(rows, sourceNames, untaggedCount) {
  const byFinal = { Pass: 0, Fail: 0, 'N/T': 0, Blocked: 0, Inconclusive: 0, 'N/A': 0 };
  let mismatch = 0;
  ...
  // D1 — N/A(범위 밖)·Inconclusive(판정 불가)는 모수에서 제외: 전체 TC = P + F + Blocked + N/T
  const totalTc = rows.length - byFinal['N/A'] - byFinal['Inconclusive'];
  // 유니크 TC — 거래소 행 분리·N/A 무관한 세트 규모 (사용자 요청 2026-07-22)
  const uniqueTc = new Set(rows.map(r => r.tcId)).size;
  ...
  return {
    totalTc, uniqueTc, byFinal, executed, passRate,
