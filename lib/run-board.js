...
// 결과형 발행 (R3 — 사용자 A안 확정 2026-07-23) — 보드의 최종(final) 칸을
// 취합 정규화 레코드로 변환한다. 웹/모바일 보드를 같은 결과형 프로젝트로 발행하면
// platform(TC ID 프리픽스)으로 구분되어 기존 취합 대시보드가 그대로 동작한다.
  ...
  for (const tc of run.tcs || []) {
    for (const ex of exchanges) {
      ...
      records.push({
        ...
        naReason: final === 'N/A' ? note : null,
        reasonNote: ['Fail', 'Blocked', 'N/A'].includes(final) ? note : null,
        title: tc.title || null,
