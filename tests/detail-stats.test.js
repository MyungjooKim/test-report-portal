...
test('축 감지: 카디널리티 낮은 범주형 컬럼(거래소·우선순위)만 축으로 채택', () => {
  const d = computeDetailStats(sheet(HEADER, ROWS), { minTotal: 1, minNonEmpty: 2, excludeAxes: [] });
...
test('체크마크 정규화 (∨/V/Y → ✓)', () => {
  ...
  ), { minTotal: 1, minNonEmpty: 2, excludeAxes: [] });
...
test('excludeAxes — 기본 제외 목록(거래소·분류 계열)은 축에서 제거', () => {
