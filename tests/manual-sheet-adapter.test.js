...
// ── P5 — TC 문서 컬럼(사전조건/스텝/기대결과) 수집 + Fail 사유 ──
const FILE_TC_DOC = {
  'TC 목록': [
    ['TC ID', '소분류', '중요도', '사전조건', '테스트 스텝', '기대결과', '결과', '특이사항'],
    ['SCM-TRAD-001', '레버리지 입력', '높음', '1. 로그인 상태\n2. Trade 화면 진입', '1. 레버리지 버튼 클릭\n2. 10x 입력', '레버리지가 10x 로 표시된다', 'Fail', '20x 로 표시됨'],
    ['SCM-TRAD-002', '주문 취소', '보통', '', '1. 주문 목록 열기', '취소 버튼이 노출된다', 'Pass', ''],
  ],
};

test('P5 — TC 문서 컬럼 수집 (사전조건/스텝/기대결과)', () => {
  const { rows } = adapter.parse(FILE_TC_DOC);
  const byId = Object.fromEntries(rows.map(r => [r.tcId, r]));
  const r = byId['SCM-TRAD-001'];
  assert.match(r.precondition, /로그인 상태/);
  assert.match(r.steps, /레버리지 버튼 클릭/);
  assert.match(r.expected, /10x 로 표시된다/);
  // Fail 사유 — 특이사항 컬럼이 reasonNote 로
  assert.strictEqual(r.reasonNote, '20x 로 표시됨');
  // Pass 행은 reasonNote 없음
  assert.strictEqual(byId['SCM-TRAD-002'].reasonNote, null);
});
